...
namespace RegIN_Kilunina.Classes
{
    public class User
    {
        ...
        public void SetUser()
        {
            ...
            if (WorkingDB.OpenConnection(mySqlConnection))
            {
                ...
                mySqlCommand.Parameters.AddWithValue("@DataCreate", this.DateCreate);
        ...
        public void CreateNewPassword()
        {
            if (Login != String.Empty)
            {
                ...
                if (WorkingDB.OpenConnection(mySqlConnection))
                {
                    WorkingDB.Query($"UPDA  TE `users` SET `Password` = '{this.Password}' WHERE `Login` = '{this.Login}'", mySqlConnection);
                    WorkingDB.CloseConnection(mySqlConnection);
